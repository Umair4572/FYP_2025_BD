"""
LGD (Loss Given Default) Regression Model Training
Trains Random Forest and XGBoost regressors to predict LGD for defaulted loans
"""
...
def load_lgd_data():
    """Load LGD data for training."""
    print("\n" + "="*80)
    print("LOADING LGD DATA")
    print("="*80)

    # Load defaulted loans with LGD
    train_path = LGD_DIR / 'defaulted_loans_lgd.csv'

    if not train_path.exists():
        print(f"ERROR: LGD data not found at {train_path}")
        print("Please run LGD simulation first: python run_lgd_simulation.py")
        return None, None, None

    df = pd.read_csv(train_path)
    print(f"\nLoaded {len(df):,} defaulted loans with LGD")

    # Remove rows with NaN LGD
    df = df.dropna(subset=['lgd'])
    print(f"After removing NaN: {len(df):,} samples")

    # Separate features and target
    target_col = 'lgd'
    exclude_cols = ['lgd', 'default', 'ead', 'ead_utilization']  # Exclude target and related columns
    ...
    print(f"LGD range: {y.min():.2%} to {y.max():.2%}")
    print(f"LGD mean: {y.mean():.2%}")
...
def save_results(rf_model, xgb_model, rf_metrics, xgb_metrics, ensemble_metrics, feature_cols):
    ...
    rf_path = MODELS_DIR / 'lgd_random_forest.pkl'
    with open(rf_path, 'wb') as f:
        pickle.dump(rf_model, f)
    print(f"✓ Random Forest saved to: {rf_path}")

    # Save XGBoost model
    xgb_path = MODELS_DIR / 'lgd_xgboost.pkl'
    ...
    metrics_path = RESULTS_DIR / 'lgd_metrics.pkl'
...
def main():
    ...
    print("\n" + "="*80)
    print("PHASE 3: LGD MODEL TRAINING")
    print("="*80)

    # 1. Load data
    print("\n1. Loading LGD data...")
    X, y, feature_cols = load_lgd_data()
